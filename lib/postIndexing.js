...
export function isPostIndexable({ post, authorUid, commentCount = 0 }) {
  ...
  // posts_record.dartに既にある手動フラグ(運営が個別投稿をnoindexにする用)。
  // 書き込むUIはネイティブ側にも無いが、フィールド自体はスキーマに存在するため尊重する。
  if (post.noindex === true) return false;
...
// business/eventsにはposts_record.dartのような手動noindexフィールドがスキーマに無いため、
// 投稿と同じ「薄いページは除外」の基準(説明文が一定量あるか、写真があるか)のみで判定する。
export function isBusinessIndexable(biz) {
  if (!biz?.display_name) return false;
  if ((biz.discription || "").length >= MIN_INDEXABLE_TEXT_LENGTH) return true;
  if (biz.photo_1) return true;
  return false;
}

export function isEventIndexable(ev) {
  if (!ev?.event_title) return false;
  if ((ev.event_description || "").length >= MIN_INDEXABLE_TEXT_LENGTH) return true;
  if (ev.event_photo) return true;
  return false;
}
